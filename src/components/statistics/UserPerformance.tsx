...
const UserPerformance = () => {
  return (
    <div className="animate-fade-in bg-gradient-to-br from-gray-900/60 to-gray-900/40 backdrop-blur-md border border-white/5 rounded-xl p-5" style={{animationDelay: '0.3s'}}>
        ...
        <div className="flex items-end gap-1 h-12">
          {[30, 45, 38, 65, 50, 55, 60, 75, 65, 80].map((height, index) => (
            <div 
              key={index}
              style={{height: `${height}%`}}
              className={`flex-1 rounded-t ${height > 60 ? 'bg-emerald-500' : 'bg-blue-500'}`}
            />
          ))}
        </div>
      </div>
